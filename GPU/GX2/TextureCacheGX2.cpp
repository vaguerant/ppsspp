...
void TextureCacheGX2::InvalidateLastTexture(TexCacheEntry *entry) {
	if (!entry || entry->texturePtr == lastBoundTexture) {
		lastBoundTexture = INVALID_TEX;
	}
}

void TextureCacheGX2::SetFramebufferSamplingParams(u16 bufferWidth, u16 bufferHeight, SamplerCacheKey &key) {
	int minFilt;
	int magFilt;
	bool sClamp;
	bool tClamp;
...
void TextureCacheGX2::BindTexture(TexCacheEntry *entry) {
	...
	GX2Sampler *sampler = samplerCache_.GetOrCreateSampler(key);
...
void TextureCacheGX2::ApplyTextureFramebuffer(TexCacheEntry *entry, VirtualFramebuffer *framebuffer) {
	...
	if (pshader) {
		...
		framebufferManagerGX2_->RebindFramebuffer();
		...
		framebufferManagerGX2_->RebindFramebuffer(); // Probably not necessary.
	}
